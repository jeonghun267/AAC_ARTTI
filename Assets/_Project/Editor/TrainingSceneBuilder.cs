...
namespace Artti.Editor
{
    public static class TrainingSceneBuilder
    {
        ...
        public static void Build(string scenePath, string scenarioId)
        {
            ...
            {
                cardSlot1 = CreateCardSlotButton("CardSlot_01", canvasGo.transform, new Vector2(-220, 280));
                cardSlot2 = CreateCardSlotButton("CardSlot_02", canvasGo.transform, new Vector2(220, 280));
            }

            // STT 풀스크린 오버레이 — 반투명 배경 + 가운데 카드(펄스 원 + 상태 + 인식 결과)
            ...
            // 원형 마스크가 없으니 그냥 사각형 — 추후 Sprite 둥근 거 넣으면 더 예쁨
