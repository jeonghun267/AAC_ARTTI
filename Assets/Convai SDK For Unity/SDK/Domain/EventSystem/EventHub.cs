    ...
    public class EventHub : IEventHub
    {
        ...
        private void InvokeSubscriber<TEvent>(IEventSubscriber<TEvent> subscriber, TEvent @event) where TEvent : notnull
        {
            ...
            catch (Exception ex)
            {
                ...
                _logger?.Error(
                    $"[EventHub] Exception in subscriber '{subscriberDescription}' for {typeof(TEvent).Name}: {ex.Message}",
                    LogCategory.Events);
                ...
                if (!_scheduler.IsMainThread())
                {
                    ...
                    {
                        _logger?.Error(
                            $"[EventHub] Background thread exception details for subscriber '{subscriberDescription}': {ex}",
                            LogCategory.Events);
                    });
                }
            }
